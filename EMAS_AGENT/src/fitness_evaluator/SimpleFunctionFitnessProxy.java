...
public class SimpleFunctionFitnessProxy implements IFitnessProxy {
	...
	private double stepResult(double[][] P) {
		...
		if (f.getValueAt(tmpPoint) <= lowestValue) {
			...
			for (int i = 0; i < n; i++) {
				tmpPoint2[i] = 3 * meanPoint[i] / 2 - pointWithHighestValue[i]
						/ 2;
			}
		}
	...
	private double[] getMeanPointInSimplex(double[][] P,
			double[] pointWithHighestValue) {
	...
	public double evaluateFitness(Genotype genotype)
		...
		try {
			...
			if (fgenotype.getP() == null || fgenotype.getMaxDomain() == null
					|| fgenotype.getMinDomain() == null)
